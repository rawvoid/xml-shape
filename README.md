...
| `valueEquality` | `null` | Optional `ValueEquality`; applied after whitespace handling. Built-ins in `ValueEqualities` (`numeric` / `dateTime` / `duration` / `bool` / `commonTypes` / `forLocalNames` / `chain`). Off by default so order numbers and similar values are not mis-parsed as numbers |
...
- **Value equality**: Literal equality by default; with `valueEquality` configured, typed equality applies only when the strategy reports a match, otherwise falls back to literal. `dateTime()` / `dateTime(ignoreTimeZone)` use ISO-8601 parsers (`OffsetDateTime` / `LocalDateTime` / `LocalDate` / `OffsetTime` / `LocalTime`). `dateTime()` is zone-aware (`Instant` equality for zoned values); `dateTime(true)` compares wall-clock fields and ignores offsets. Same temporal kind only (e.g. LocalDate and Instant do not equate). `duration()` uses `java.time.Duration` (`P1D` ≡ `PT24H`; year/month forms fall through to literal). `commonTypes()` chains `dateTime(true)`, duration, numeric, and bool.

...
- **Value equality**: Literal equality by default; with `valueEquality` configured, typed equality applies only when the strategy reports a match, otherwise falls back to literal. `dateTime()` / `dateTime(ignoreTimeZone)` use ISO-8601 parsers (`Instant` / `OffsetDateTime` / `LocalDateTime` / `LocalDate` / `OffsetTime` / `LocalTime`). Default is zone-aware (`Instant` equality for zoned values). `dateTime(true)` compares wall-clock fields and ignores offsets. Same temporal kind only (e.g. LocalDate and Instant do not equate).
